...
def test_inventory_title(driver_logged):
    ...
    assert titulo == "Swag Labs", "Error: El título de la página a la que se accede no es correcto"
...
def test_ui_elements(driver_logged):
    ...
    assert menu.is_displayed(), "Error: El ícono del menú no está presente"
    assert filtro.is_displayed(), "Error: El filtro del catálogo no esta presente"